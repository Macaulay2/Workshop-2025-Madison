...
- Post-Direction A (Increment 4), every strategy — Default, Larsson, MatrixHi-mode (`Strategy => "Greedy", AdjustParams => false`), Greedy — skips the graph-ideal extension on monomial actions and builds the template on the `[E | R | B]` layout, saving $|B|$ rows. Polynomial actions lift to $R_s = R[s]/⟨s-a⟩$ (Increment 3) and take the same path.
